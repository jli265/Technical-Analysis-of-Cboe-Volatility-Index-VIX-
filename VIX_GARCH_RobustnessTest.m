...
%Construct training period with data before May.  
r_train=r(1:end,:);
...
theta_initial=[-0.008,0.6,0.4,0]; % Initial conditions 
...
outcome = table(parameter_names,theta',round(sqrt(diag(asy_var)),6), round(t_stats,6),...
...
%-----------------------------------------------------------------
% II.	Use Monte Carlo Simulations to Predict Future Returns on May 8th
%-----------------------------------------------------------------

T_test=14; % 6 days later
N=1e6;
h_test=[h(end-8,1)*ones(1,N);zeros(T_test,N)];
epsilon=[(r_train(end-8,1)-beta)*ones(1,N);zeros(T_test,N)];

r_test=[r_train(end-8,1)*ones(1,N) ; zeros(T_test,N) ];
vix=[prices(end-8,1)*ones(1,N); zeros(T_test,N)];
...
grid on;
vix_mix=[ prices(end-8:end,1) ; real(mean(vix(10:15,:),2)) ];
for  i=0:14
